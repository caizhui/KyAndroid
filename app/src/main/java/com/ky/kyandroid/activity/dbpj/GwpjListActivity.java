...
import com.ky.kyandroid.adapter.GwpjListAdapter;
...
public class GwpjListActivity extends AppCompatActivity {
     ...
     * 事件列表
     */
    private List<GwpjEntity> gwpjEntityList;


    private GwpjListAdapter adapter;


    /**
     * 部门评价实体
     */
    GwpjEntity gwpjEntity;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                // 默认处理
                case 0:
                    Toast.makeText(GwpjListActivity.this, message, Toast.LENGTH_SHORT).show();
                    break;
                // 加载失败
                case 1:
                    Log.i(TAG, "刷新操作...");
                    if (handleResponseData(message)) {
                        ...
                        Toast.makeText(GwpjListActivity.this, "刷新成功", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(GwpjListActivity.this, "刷新失败", Toast.LENGTH_SHORT).show();
                    }
                ...
                case 7:
                    ...
                    if (handleResponseData(message)) {
                        ...
                        Toast.makeText(GwpjListActivity.this, "查询不到符合条件记录", Toast.LENGTH_SHORT).show();
                    }
                    break;
                case 8:
                    // 状态修改
                    ...
                    if (handleResponseDataState(message)) {
                        Toast.makeText(GwpjListActivity.this, "操作成功", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(GwpjListActivity.this, "操作失败", Toast.LENGTH_SHORT).show();
                    }
                    break;
    ...
    void initViewAndEvent() {
        ...
        sweetAlertDialogUtil = new SweetAlertDialogUtil(GwpjListActivity.this);
        gwpjEntityList = new ArrayList<GwpjEntity>();
    ...
    private void initListView() {
        ...
        adapter = new GwpjListAdapter(gwpjEntityList, GwpjListActivity.this);
                ...
                if (AckMessage.SUCCESS.equals(ackMsg.getAckCode())) {
                    ...
                    if(pageBean!=null){
                        ...
                        if (dataL != null && dataL.size() > 0) {
                            ...
                            dataList = (List<GwpjEntity>) JsonUtil.fromJson(resultList, new TypeToken<List<GwpjEntity>>() {
                            });
                            flag = true;
                        } else {
                            Log.i(TAG, "解释列表信息失败...");
    ...
    private void notifyListViewData(boolean isAdd) {
        entityList = (List<GwpjEntity>) dataList;
        if (isAdd) {
            // 追加列表
        ...
        centerText.setText("岗位评价(" + adapter.getCount() + ")");
    }
    ...
    private void updateListDataStatie(SjHandleParams sjHandleParams) {
        List<GwpjEntity> tFtDbEntityList = adapter.getList();
        if (tFtDbEntityList != null) {
            for (int i = 0; i < tFtDbEntityList.size(); i++) {
                GwpjEntity entity = tFtDbEntityList.get(i);
            }
    ...
    public void OnItemClick(int position) {
       /* GwpjEntity = (GwpjEntity) adapter.getItem(position);
        Intent intent = new Intent(this, BmpjDetailActivity.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable("GwpjEntity", GwpjEntity);
