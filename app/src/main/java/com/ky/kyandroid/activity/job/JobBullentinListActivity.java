...
import com.ky.kyandroid.adapter.JobBulletinListAdapter;
...
public class JobBullentinListActivity extends AppCompatActivity {
     ...
     * List列表
     */
    @BindView(R.id.search_job_list)
    ListView searchJobList;
     ...
     * 事件列表
     */
    private List<TFtAppReportEntity> jobEntityList;


    private JobBulletinListAdapter adapter;
    ...
    List<TFtAppReportEntity> entityList;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                // 默认处理
                case 0:
                    Toast.makeText(JobBullentinListActivity.this, message, Toast.LENGTH_SHORT).show();
                    break;
                // 加载失败
                case 1:
                    Log.i(TAG, "刷新操作...");
                ...
                case 7:
                    ...
                    if (handleResponseData(message)) {
                        ...
                        Toast.makeText(JobBullentinListActivity.this, "查询不到符合条件记录", Toast.LENGTH_SHORT).show();
                    }
                    break;
                case 8:
                    // 状态修改
                    ...
                    if (handleResponseDataState(message)) {
                        Toast.makeText(JobBullentinListActivity.this, "操作成功", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(JobBullentinListActivity.this, "操作失败", Toast.LENGTH_SHORT).show();
                    }
                    break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.left_btn:
                ...
                break;
            case R.id.right_btn:
                Intent intent = new Intent(this, JobBulletinAddActivity.class);
    ...
    void initViewAndEvent() {
        ...
        sweetAlertDialogUtil = new SweetAlertDialogUtil(JobBullentinListActivity.this);
        jobEntityList = new ArrayList<TFtAppReportEntity>();
        userId = sp.getString(USER_ID, "");
        centerText.setText("工作简报");
    ...
    void initBundle() {
        ...
        // 设置标题及颜色
        centerText.setText("工作简报(" + total + ")");
        //toolbar_layout.setBackgroundColor(Color.parseColor("#A4C639"));
        swipeRefreshUtil = new SwipeRefreshUtil(swipeContainer, Constants.SERVICE_QUERYREPORTLIST, mHandler);
        // 上拉刷新初始化
    ...
    private void initListView() {
        ...
        searchJobList.addFooterView(list_jiazai, null, false);
        searchJobList.setSelector(getResources().getDrawable(R.drawable.item_selector_grey));
        //滚动监听
        searchJobList.setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
        ...
        adapter = new JobBulletinListAdapter(jobEntityList, JobBullentinListActivity.this);
        searchJobList.setAdapter(adapter);
    }
                ...
                if (AckMessage.SUCCESS.equals(ackMsg.getAckCode())) {
                    ...
                    if(pageBean!=null){
                        ...
                        if (dataL != null && dataL.size() > 0) {
                            ...
                            dataList = (List<TFtAppReportEntity>) JsonUtil.fromJson(resultList, new TypeToken<List<TFtAppReportEntity>>() {
                            });
                            flag = true;
                        } else {
                            Log.i(TAG, "解释列表信息失败...");
    ...
    private void notifyListViewData(boolean isAdd) {
        entityList = (List<TFtAppReportEntity>) dataList;
        if (isAdd) {
            // 追加列表
        ...
        centerText.setText("工作简报(" + adapter.getCount() + ")");
    }
    ...
    private void updateListDataStatie(SjHandleParams sjHandleParams) {
        List<TFtAppReportEntity> tFtDbEntityList = adapter.getList();
        if (tFtDbEntityList != null) {
            for (int i = 0; i < tFtDbEntityList.size(); i++) {
                tFtAppReportEntity = tFtDbEntityList.get(i);
            }
     ...
     * @param position
     */
    @OnItemClick(R.id.search_job_list)
    public void OnItemClick(int position) {
        tFtAppReportEntity = (TFtAppReportEntity) adapter.getItem(position);
        Intent intent = new Intent(this, JobBulletinDetailActivity.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable("tFtAppReportEntity",tFtAppReportEntity );
